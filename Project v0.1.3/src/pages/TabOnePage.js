...
import { IonItem, IonContent, IonText, IonList, IonLabel, IonItemSliding, IonItemOption, IonItemOptions} from "@ionic/react";
...
class TabOnePage extends Component {
    ...
    return this.props.store.itemEntries.map(([key, value]) => {
      return (
        <IonItemSliding key={key}>
          <IonItem onClick={e => { if (!e.currentTarget) { return; } e.preventDefault();
          ...
          <IonItemOptions side="end">
            <IonItemOption onClick={e => this._delete(e, value)} color="danger">Delete</IonItemOption>
          </IonItemOptions>
        </IonItemSliding>
      );
    });
  ...
  render() {
    ...
    return (
      <>
        <AddItemModal showModal={showAddItemModal} onDidDismiss={_v => {
            if (_v) {
