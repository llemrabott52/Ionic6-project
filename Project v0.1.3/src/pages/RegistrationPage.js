...
class RegistrationPage extends Component {
  constructor(props) {
    super(props);
    this.state = { showErrorToast: false, errMsg: true};
  ...
  render() {
    return (
      <>
        <BasicPage title="Create Account Page" hasMenu backAction={() => {}} renderContent={history => {
            return (
              <>
                <IonItem>
                ...
                <div style={{ padding: 8 }}>
                  <IonButton expand="full" style={{ margin: 14 }} onClick={e => {
                      if (!e.currentTarget) {return;} e.preventDefault(); this._doCreateAccount(history);}}>
                    Create Account
                  </IonButton>
                  <IonButton expand="full" style={{ margin: 14 }} onClick={e => { e.preventDefault(); history.goBack();}}>
                    Cancel
                  </IonButton>
                </div>
              </>
